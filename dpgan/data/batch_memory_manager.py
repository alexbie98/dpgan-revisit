...
class BatchSplittingSampler(Sampler[List[int]]):
    ...
    def __init__( # pylint: disable=super-init-not-called
        ...
        *,
        ...
        self.num_physical_batches = None
        self.current_logical_batch_size = None

    def __iter__(self):
        for batch_idxs in self.sampler:
            self.current_logical_batch_size = len(batch_idxs)
            split_idxs = np.array_split(
                batch_idxs, math.ceil(len(batch_idxs) / self.max_batch_size)
            )
            self.num_physical_batches = len(split_idxs)
